...
var lesson_plan_toptext = singleton.pronounsLessonPlanTop
var lesson_plan_bottomtext = singleton.pronounsLessonPlanBottom
...
func _ready():
	...
	get_node("end_chapter_challenge").questions = singleton.nounsQuestions # change to pronoun questions when complete
	...
	if pronounsScreenNode.is_visible(): # navigate the lesson plan
		if right_trigger or pronounsScreenNode.get_node("right_button").is_pressed():
			if lesson_plan_page < lesson_plan_toptext.size()-1: 
				lesson_plan_page += 1
				pronounsScreenNode.get_node("intro_text").set_bbcode(lesson_plan_toptext[lesson_plan_page])
